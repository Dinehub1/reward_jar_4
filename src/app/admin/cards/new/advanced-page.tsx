...
interface AdvancedCardCreationProps {
  businesses: Business[]
  onSwitchToQuick: () => void
  preSelectedBusinessId?: string
}

export default function AdvancedCardCreation({
  businesses,
  onSwitchToQuick,
  preSelectedBusinessId
}: AdvancedCardCreationProps) {
  ...
  // Auto-populate business info when preselected
  useEffect(() => {
    if (preSelectedBusinessId) {
      const business = businesses.find(b => b.id === preSelectedBusinessId)
      if (business) {
        setCardData(prev => ({
          ...prev,
  ...
  const saveCard = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        router.push('/admin/cards?created=true&mode=advanced')
      } else {
  ...
  // Render step content - This contains all the original advanced form logic
